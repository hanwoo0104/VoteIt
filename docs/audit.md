...
- Supabase를 제거하고 `data/voteit.sqlite` 로컬 DB 파일 기반으로 변경했습니다.
- Node 내장 `node:sqlite`와 HTTP-only cookie 세션으로 회원가입/로그인/로그아웃을 처리합니다.
- `users`, `profiles`, `sessions`, `phone_verifications`로 인증/프로필/검증 상태를 분리했습니다.
- 현안, 의견, 투표, 조회, 댓글, 좋아요, 신고, 정치인, 채팅, 메시지, 뉴스 링크를 SQLite API route로 연결했습니다.
- 투표/댓글/채팅 관련 카운트는 SQLite transaction 안에서 즉시 갱신합니다.
- 실제 투표자 데이터로 연령/성별/지역/소득 익명 통계를 계산합니다.
- 채팅은 로컬 DB 저장 + 짧은 폴링 갱신 구조로 동작합니다.
- 관리자 페이지는 실제 SQLite DB upsert/delete/report resolve 서비스를 사용합니다.
